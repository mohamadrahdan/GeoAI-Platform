...
def ingest_local_files(inp: LocalIngestionInput, dm: BaseDataManager) -> str:
    ...
    if not files:
        raise ValueError(
            f"No files found in {inp.source_dir} using pattern {inp.glob_pattern}"
        )
    ...
    with UnitOfWork() as uow:
        ...
        manifest_uri = dm.write_text(
            manifest_relpath, json.dumps(manifest, ensure_ascii=False, indent=2)
        )
        ...
        logger.info(
            ...
            extra={"dataset_id": ds.id, "run_id": run.id, "files": len(files)},
        )
